...
phenoData <- data.frame('BiosampleId' = libnames) %>%
    plyr::join(y = meta1.bysample, by = 'BiosampleId', type = 'left')  %>%
...
allMeta <- data.frame('BiosampleId' = libnames) %>%
    plyr::join(y = meta1.bysample, by = 'BiosampleId', type = 'left')  %>%
    plyr::join(y = meta2, by = 'BiosampleId', type = 'left') %>%
    set_rownames(libnames)
